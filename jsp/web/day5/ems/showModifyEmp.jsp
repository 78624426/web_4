...
<body>
    ...
    <form action="${pageContext.request.contextPath}/modify.emp" method="post">
        ...
        兴趣:<input type="checkbox" name="hobbies" value="运动" <c:if test="${f:contains(emp.hobbies,'运动')}">checked</c:if>   >运动
        <input type="checkbox" name="hobbies" value="旅游" <c:if test="${f:contains(emp.hobbies,'旅游')}">checked</c:if> >旅游
        <input type="checkbox" name="hobbies" value="电影" <c:if test="${f:contains(emp.hobbies,'电影')}">checked</c:if> >电影
        <br>
